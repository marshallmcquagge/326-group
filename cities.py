...
class Cities:
    """
    Catalog of cities, finds closest cities to a given reference point.
    
    Attributes: 
            cities: (list of dict): Contains information about the cities
    ...
    def __init__(self, filepath):
        ...
        Args:
            filepath (str): path to a file containing city data
            
        Side effects:
            Creates the cities attibute
        """
        self.cities = []
    ...
    def nearest(self, lat, lon, min_population=0, n=10):
        """
        Finds the closest cities to the coordinates with a population greater
        than or equal to the minimum.
        
        Args:
            lat (float): latitude in decimal degrees
            lon (float): longitude in decimal degrees
            min_population (int, optional): a minimum population to return
            values of, default value of 0
            n (int, optional): maximum number of results to return, default
            value of 10
            
        Returns:
            Return a list of the 'n' closest cities with populations greater than
        or equal to 'min_population'. 
        """
        cities = [city for city in self.cities if city["pop"] >= min_population]
        cities.sort(key=lambda c: get_dist((lat, lon), (c["lat"], c["lon"])))
        return cities[:n]
